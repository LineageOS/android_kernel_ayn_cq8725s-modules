...
def _define_module_for_target_variant_chipset(target, variant, chipset):
    ...
    deps = select({
        ...
        "//build/kernel/kleaf:socrepo_false": ["//vendor/ayn/kernel:all_headers"],
    })
    ...
    if target == "neo-la":
        kernel_build = select({
            ...
            "//conditions:default": "//vendor/ayn/kernel:{}".format(tv),
        })
    else:
        kernel_build = select({
            "//build/kernel/kleaf:socrepo_true": "//soc-repo:{}_base_kernel".format(tv),
            "//build/kernel/kleaf:socrepo_false": "//vendor/ayn/kernel:{}".format(tv),
        })
